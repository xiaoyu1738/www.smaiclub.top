...
export function HomePage({ onMinimize }: HomePageProps) {
  ...
  const [duration, setDuration] = useState(initialSnapshot.duration);
  ...
  return (
    <section
      className="smai-player-page relative min-h-[calc(100vh-88px)] bg-background-dark text-text-dark md:h-[calc(100vh-88px)] md:overflow-hidden"
    >
      <div
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col gap-4 px-4 pb-4 md:grid md:grid-rows-[1fr_auto] md:px-8">
        <main className="flex min-h-0 flex-1 flex-col gap-6 pt-4 md:grid md:grid-cols-2 md:gap-10 md:pt-6">
          <div className="flex min-h-0 flex-col items-center justify-center">
            ...
            <div className="relative aspect-square w-full max-w-[280px] overflow-hidden rounded-2xl shadow-2xl shadow-black/60 md:max-w-md">
              <img
                alt="Rock Anthem Album Cover"
                className="h-full w-full object-cover"
                src={track.cover}
              />
            </div>
            <div className="mt-5 w-full max-w-md text-center">
              <h1 className="mb-2 truncate text-3xl font-bold tracking-tight md:text-4xl">
                {track.title}
              </h1>
              <p className="mb-1 text-lg font-medium text-subtext-dark md:text-xl">
                {track.artist}
              </p>
