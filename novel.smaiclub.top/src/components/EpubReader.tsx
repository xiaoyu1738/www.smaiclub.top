    ...
    useEffect(() => {
        ...
        const loadBook = async () => {
            try {
                ...
                // 确保URL被正确编码，以处理非ASCII字符
                const encodedUrl = encodeURI(readingInfo.epubUrl);
                const epubBook = ePub(encodedUrl);
                ...
                const timeoutPromise = new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('书籍解析超时。可能是文件损坏或网络问题。')), 20000) // 20秒超时
                );
